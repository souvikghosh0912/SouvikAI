...
/** Fire-and-forget persistence for a `<remember>`-extracted fact. Dedup/cap enforcement lives server-side. */
function saveMemoryAsync(content: string): void {
    fetch('/api/settings/memory', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content, source: 'auto' }),
    }).catch((e) => console.error('[useChat] Failed to save auto-extracted memory:', e));
}
    ...
    const sendMessage = useCallback(async (content: string, attachments: Attachment[] = [], tool?: string) => {
        ...
        try {
            ...
            const finalContent = await runChatCompletion({
                ...
                onMemoriesExtracted: (facts) => {
                    facts.forEach((fact) => saveMemoryAsync(fact));
                },
            });
