...
/** Hard cap on a single memory entry's length (mirrors the DB check constraint). */
export const MAX_MEMORY_CONTENT_CHARS = 500;

/** Max memory rows kept per user — oldest is evicted on overflow. */
export const MAX_MEMORIES_PER_USER = 100;

/** Max memories injected into the system prompt per chat turn. */
