...
/**
 * Edits an image using the NVIDIA NIM flux.1-kontext-dev model via the
 * native GenAI API.
 *
 * Endpoint: POST https://ai.api.nvidia.com/v1/genai/black-forest-labs/flux.1-kontext-dev
 * Response: { artifacts: [{ base64: "..." }] }
 *
 * @returns A data-URL string (`data:image/jpeg;base64,…`) ready for an <img> tag.
 */
export async function editNvidiaImage(
    prompt: string,
    imageB64: string,
    options: {
        signal?: AbortSignal;
    ...
    // The API expects a full data URL, e.g., "data:image/png;base64,..."
    // Ensure imageB64 is prefixed properly if it isn't already.
    ...
    if (!response.ok) {
        const errorText = await response.text().catch(() => response.statusText);
        throw new Error(`NVIDIA image edit API error (${response.status}): ${errorText}`);
    }
    ...
    const b64 = data.artifacts?.[0]?.base64;
    if (!b64) {
        throw new Error('NVIDIA image edit API returned no image data');
    }
